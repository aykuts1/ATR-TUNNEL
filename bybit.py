"""
bybit.py - Bybit Unified Trading v5 API wrapper.
pybit kullanir. Sadece bot icin gerekli fonksiyonlar.
"""
import time
...
class BybitClient:
    ...
    def get_instrument_info(self, symbol: str) -> dict:
        """tickSize, qtyStep, minOrderQty ve maxLeverage bilgileri."""
        ...
        lev_filter = it.get("leverageFilter", {})
        info = {
            "symbol": symbol,
            ...
            "max_leverage": float(lev_filter.get("maxLeverage", "1")),
        }
        self._instrument_cache[symbol] = info
        return info

    def supports_leverage(self, symbol: str, leverage: int) -> bool:
        info = self.get_instrument_info(symbol)
        return info["max_leverage"] >= leverage

    def set_leverage(self, symbol: str, leverage: int) -> None:
        """Isolated mod, hem buy hem sell icin ayni kaldirac."""
        try:
            self._retry(self.session.set_leverage,
                        category=CATEGORY, symbol=symbol,
                        buyLeverage=str(leverage), sellLeverage=str(leverage))
        except Exception as e:
            # Bybit "leverage not modified" donerse normal
            if "not modified" in str(e).lower() or "leverage not modified" in str(e).lower():
                return
            raise

    def switch_isolated(self, symbol: str, leverage: int) -> None:
        """Isolated marjine gecir. Zaten oyle ise gormezden gel."""
        try:
            ...
            if "not modified" in msg or "already" in msg or "110026" in msg:
                return
            # Sessiz gec - bazi hesaplarda zaten isolated
            return

    # --- Fiyat & klines ----------------------------------------------------
    def fetch_kline(self, symbol: str, interval: str, limit: int = 200) -> List[List]:
        """[timestamp, open, high, low, close, volume, turnover] listesi.
        Bybit en yeni mumu basta verir; cagiran taraf siralayabilir."""
    ...
    def fetch_positions(self) -> List[dict]:
        """Tum acik USDT perpetual pozisyonlar."""
    ...
    def place_limit_post_only(
        ...
        self._check(resp, f"place_order {symbol} {side}")
    ...
    def cancel_order(self, symbol: str, order_id: str) -> None:
        try:
            ...
            pass  # zaten kapalı/dolmuş olabilir
    ...
    def set_position_stop_loss(self, symbol: str, sl_price: str) -> None:
        """Mevcut pozisyona borsa-tarafli stop loss yerlestir."""
        try:
            ...
            # not modified ise gormezden gel
